#!/usr/bin/env python3
"""
Step 4: Website Enrichment for Atlanta Dentists
...
def write_enriched_csv(path, rows, fields):
...
def main():
    ...
    print("=" * 60)
    print("Atlanta Dental Clinics — Website Enrichment")
    ...
    # Detect input fields dynamically
    with open(INPUT_CSV, "r", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        input_fields = list(reader.fieldnames)

    # Build output fields: input fields + enrichment fields (no dupes)
    all_fields = list(input_fields)
    ...
    with sync_playwright() as p:
        ...
        for i, row in enumerate(rows):
            ...
            if (i + 1) % 50 == 0:
                write_enriched_csv(OUTPUT_CSV, enriched_rows, all_fields)
