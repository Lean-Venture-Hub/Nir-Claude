#!/usr/bin/env python3
"""
Step 2: Website Enrichment for Charlotte Med Spas
Visits each clinic's website to extract:
  - Social media URLs (Facebook, Instagram, TikTok)
  - Provider/staff count (from About/Team pages)
  - Site quality score (0-10)
  - Online booking detection

Usage:
  python3 02_enrich_websites.py                # Full run
  python3 02_enrich_websites.py --test         # First 5 clinics
  python3 02_enrich_websites.py --start=100    # Resume from row 100
"""
...
# Booking platform signatures — med spa specific + generic
BOOKING_SIGNATURES = [
    # Med spa booking platforms
    "vagaro", "glossgenius", "boulevard", "aesthetic record", "patientnow",
    "nextech", "symplast", "mangomint", "zenoti",
    # Generic booking signals
    "book online", "book now", "schedule appointment", "schedule online",
    "request appointment", "book appointment",
    "calendly.com", "acuityscheduling.com", "setmore.com",
    "zocdoc.com", "zocdoc",
    "patientpop", "getweave.com",
]
...
# Provider/doctor indicators — med spa titles
DOCTOR_TITLES = [
    r'\bMD\b', r'\bDO\b', r'\bNP\b', r'\bPA\b', r'\bRN\b',
    r'\bBSN\b', r'\bAPRN\b', r'\bFNP\b', r'\bDNP\b', r'\bCRNA\b',
    r'\bM\.D\.\b', r'\bD\.O\.\b',
    r'\bDr\.\s', r'\bDoctor\b',
]
TEAM_LINK_PATTERNS = [
    "about", "team", "staff", "doctors", "providers", "our-team",
    "meet-the-team", "meet-our-team", "our-doctors", "about-us",
    "practitioners", "injectors", "aestheticians", "our-providers",
    "specialists",
]
...
def enrich_clinic(page, row):
    ...
    if doctor_count > 5:
        comment_parts.append(f"Large team ({doctor_count} providers)")
    elif doctor_count == 1:
...
def main():
    ...
    print("=" * 60)
    print("Charlotte Med Spas — Website Enrichment")
    ...
    if not os.path.exists(INPUT_CSV):
        ...
        print("Run 01_scrape_google_maps.py first.")
        sys.exit(1)

    rows = load_csv_rows(INPUT_CSV)
    print(f"Loaded {len(rows)} med spas from {INPUT_CSV}")

    enriched_keys = set()
    if os.path.exists(OUTPUT_CSV) and start_idx > 0:
        existing = load_csv_rows(OUTPUT_CSV)
        enriched_keys = {(r["name"], r["address"]) for r in existing}
        print(f"Loaded {len(enriched_keys)} already-enriched med spas")

    if test_mode:
        rows = rows[:5]

    with_website = sum(1 for r in rows if r.get("website", "").strip())
    print(f"Med spas with websites: {with_website}/{len(rows)}")
    ...
    with sync_playwright() as p:
        ...
        for i, row in enumerate(rows):
            ...
            try:
                ...
                booking = " booking" if enrichment.get("has_online_booking") == "yes" else ""
                social = " social" if any([enrichment["facebook_url"], enrichment["instagram_url"]]) else ""
    ...
    print(f"\n{'='*60}")
    print(f"DONE!")
    print(f"Total med spas enriched: {len(enriched_rows)}")
