#!/usr/bin/env python3
"""
Add review_recency column to labeled-medspas.csv.
...
Usage:
  python3 05_add_review_recency.py                # Full run
  python3 05_add_review_recency.py --test         # First 10 clinics
  python3 05_add_review_recency.py --start=500    # Resume from row 500
"""
...
INPUT_CSV = os.path.join(SCRIPT_DIR, "labeled-medspas.csv")
...
def main():
    ...
    print("=" * 60)
    print("Review Recency Analysis — Charlotte Med Spas")
    ...
    print(f"Need to scrape: {len(need_scrape)} med spas (skipping {len(rows_to_process) - len(need_scrape)})")
