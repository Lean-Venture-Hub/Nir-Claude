...
- Visual restraint: Maximum 3-4 distinct visual elements. Less is more.
...
### People (MANDATORY)
- Replace ALL people in template with DIFFERENT individuals matching ICP and brief tone
...
**Minimalism = intentional restraint, not boring emptiness.**

### Element Count Rule

- Target: 2-3 primary elements maximum
- Primary elements = headline, ONE hero visual (image/illustration/graphic), CTA, logo
- If you have more than 4 elements competing for attention → remove the weakest

### What Minimalism IS

- Bold single image with short headline
- Dramatic typography with ample whitespace
- One striking graphic element + sparse text
- High contrast with breathing room
- Strong brand color used sparingly but confidently

### Copy Minimalism

- Headline: 3-6 words ideal (max 8)
- Body: AVOID if possible; only if absolutely necessary for comprehension
- CTA: 2 words ideal (max 3)
...
**Surprise often comes from RESTRAINT, not addition.** The most surprising choice may be what you REMOVE or leave empty.

### Approved Surprise Techniques

**Restraint-based (prioritize these for minimalist impact):**
...
- **Scale Contrast**: ONE dramatically oversized or undersized element
...
- do not tint or overlay a brand color over images or visuals.
...
- Do not use the same person/face from the template—always replace with a new individual.
...
3. **Analyze TemplateElements** → Locate old logos, icons, people, and all branding elements that need to be removed or replaced. Understand all elements that need to change.
...
6. **Replace People** → If template has any human subjects, replace with new individuals matching ICP and brief tone.

7. **Insert BrandLogo** → Place unaltered; verify clear-space.

8. **Replace Content** → Replace template copy & imagery per brief. Keep element count minimal (2-3 primary elements).

9. **Apply Surprise Elements** → Based on brief type (see Surprise Mandate section). Prefer restraint-based surprises.

10. **Run Quality Checks** → Execute accessibility & integrity audit; fix if failed.

11. **Final Verification** → Spell-check, grammar check, contrast check, surprise effectiveness check.
...
- subtle to moderate look and Feel of the brand palette.
- Replace any people with new individuals matching ICP
- ONE understated surprise (e.g., testimonial quote in unexpected small type at edge)
- Professional, credible, trustworthy feel
- 2-3 primary elements only
...
- Replace any people with new individuals matching ICP and bold/confident tone
- Multiple integrated surprises (e.g., empty-focus + scale contrast working together)
- Disruptive, attention-grabbing, confident feel
- Still restrained: 3-4 elements maximum, not cluttered
...
Also identify any people in the template. All human subjects MUST be replaced with different individuals—never keep the same person from the source template.
